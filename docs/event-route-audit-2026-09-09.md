# Event and route audit — September 9, 2026

Source: [master document, Events List tab](https://docs.google.com/document/d/1gRzKqc7Layg1XinjCOBbR8VhDfNr6JAf5Jpv-fSETR0/edit?tab=t.n9whrzjq5eor). Re-fetched in the final recovery session and independently parsed from the live text export; all 19 records exactly match the audit fixture. The supplied URL opens the Donate Form tab; the Events List tab in the same document is the event source.

With the subsequent updates documented below, all 19 published events match the maintained fixture for title (ignoring capitalization), date, campaign day number, time, venue, description, and supplied RSVP URL. The app includes all 19 supplied RSVP URLs and the additional Lakeville signup link embedded in the description. Event dates are shown prominently in the schedule and map details. Campaign days count calendar dates from September 22 (day 1) to November 1 (day 41).

## Event comparison

Times below are Central Time. “Pending” means the master has no RSVP URL.

| Stop | Day | Event | Date | Time | Venue | RSVP |
| --- | --- | --- | --- | --- | --- | --- |
| 1 | 1 | Ely Launch Event | 2026-09-22 | 5:00 PM – 7:00 PM | Puddle Jumper Park | [RSVP](https://secure.everyaction.com/bRWw6xnPwUWh8Xw4QzDwyA2) |
| 2 | 4 | Heck Fest | 2026-09-25 | 5:00 PM – 7:00 PM | Spokengear | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-heck-fest-two-harbors) |
| 3 | 5 | Duluth Public Lands Day Celebration | 2026-09-26 | 3:00 PM – 5:30 PM | Duluth Cider | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-duluth-public-lands-day-celebration) |
| 4 | 7 | Cloquet Door Knock for BWCA Champions Mary Carlson and Chris Swanson | 2026-09-28 | 4:00 PM – 6:00 PM | Cloquet, MN | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-cloquet-door-knock-bwca-champions-mary-carlson-and-chris-swanson) |
| 5 | 9 | Grand Rapids Community Gathering | 2026-09-30 | 5:00 PM – 7:00 PM | Klockow Brewing | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-grand-rapids-community-gathering-stop) |
| 6 | 12 | Bemidji Door Knock BWCA Champion Reed Olson | 2026-10-03 | 1:00 PM – 3:00 PM | DFL Field Office | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-bemidji-door-knock-bwca-champion-reed-olson) |
| 7 | 16 | Moorhead Community Gathering | 2026-10-07 | 6:00 PM – 7:30 PM | Swing Barrel Brewing | [RSVP](https://secure.everyaction.com/RBBB-EOuyU238YN4wqLq6A2) |
| 8 | 21 | Saint Cloud Door Knock for BWCA Champs | 2026-10-12 | 5:00 PM | St. Cloud | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-saint-cloud-door-knock-bwca-champs) |
| 9 | 24 | Mankato Community Gathering | 2026-10-15 | 6:00 PM – 8:00 PM | LocAle Brewery | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-mankato-community-gathering) |
| 10 | 26 | Northfield Community Gathering | 2026-10-17 | 1:00 PM – 3:00 PM | Seeds Farm | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-northfield-community-gathering) |
| 11 | 28 | Rochester Community Gathering | 2026-10-19 | 6:00 PM – 8:00 PM | Cascade Meadows Wetlands and Environmental Science Center | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-rochester-community-gathering) |
| 12 | 29 | Winona Community Gathering | 2026-10-20 | 6:00 PM – 8:00 PM | Two Fathoms Brewing | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-winona-community-gathering) |
| 13 | 31 | Lake City Community Gathering | 2026-10-22 | 6:30 PM – 8:00 PM | Lake City Public Library | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-lake-city-author-talk-dave-and-amy) |
| 14 | 33 | Lakeville Door Knock for BWCA Champs | 2026-10-24 | 1:00 PM – 3:00 PM | DFL Field Office - Burnsville | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-lakeville-door-knock-bwca-champs) |
| 15 | 34 | Chaska Door Knock for BWCA Champs | 2026-10-25 | 1:00 PM – 3:00 PM | Firemans Park | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-chaska-door-knock-bwca-champs) |
| 16 | 35 | Minnetonka Community Gathering | 2026-10-26 | 6:00 PM – 8:00 PM | Back Channel Brewing | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-minnetonka-community-gathering) |
| 17 | 36 | Coon Rapids Door Knock for BWCA Champs | 2026-10-27 | 4:00 PM – 6:00 PM | TBD | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-coon-rapids-door-knock-bwca-champs) |
| 18 | 38 | Saint Paul Community Gathering | 2026-10-29 | 6:30 PM – 8:00 PM | Patagonia | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-st-paul-community-gathering) |
| 19 | 41 | Stillwater Grand Finale | 2026-11-01 | 3:00 PM – 5:00 PM | River Siren Brewery | [RSVP](https://give.savetheboundarywaters.org/a/pedal-polls-stillwater-grand-finale) |

## Source gaps and inconsistencies

- Ely: the master now supplies an RSVP URL, added in the subsequent Ely update below. A September 21 campaign update moved the start time to 5:00 PM.
- Saint Cloud: location is “St. Cloud,” with no street address; the subsequent campaign update supplies 5:00 PM. Coon Rapids: venue remains TBD. Cloquet: only “Cloquet, MN” is supplied, with no street address.
- Saint Paul: the document supplies 6:30–8:00 PM, despite the original request describing its time as an exception. The app follows the document.
- Chaska: the master title and venue say Chaska/Firemans Park, while the description says Chanhassen. The app preserves the master copy.
- Lake City: the new title and description say community gathering; the supplied RSVP URL still contains “author-talk-dave-and-amy.” The app uses the exact supplied URL.

## Route comparison

All six route legs and 32 schedule rows remain present, in their existing order. Every event falls within its assigned route leg’s date range. Leg 1 now starts September 22 so it includes the Ely launch. The master Events List does not supply revised daily cycling legs or GPX tracks; daily travel dates, endpoints, distances, and geometry were checked against the repository’s route manifest, CSV, and GPX files. They cannot all be independently confirmed from the event-only master.

| Leg | Route | Dates | Schedule rows | Events |
| --- | --- | --- | --- | --- |
| Leg 1 | Ely to Grand Rapids | 2026-09-22–2026-09-30 | 6 | 5 |
| Leg 2 | Grand Rapids to Detroit Lakes | 2026-10-01–2026-10-05 | 5 | 1 |
| Leg 3 | Moorhead to Saint Cloud | 2026-10-06–2026-10-11 | 4 | 1 |
| Leg 4 | Saint Cloud to Mankato | 2026-10-12–2026-10-15 | 2 | 2 |
| Leg 5 | Mankato to Winona | 2026-10-16–2026-10-20 | 5 | 3 |
| Leg 6 | Winona to Stillwater | 2026-10-21–2026-11-01 | 10 | 7 |

Event dates and existing travel dates differ on these cards; the app labels the travel dates separately:

| Event | Current event date | Existing ride dates |
| --- | --- | --- |
| Ely Launch Event | 2026-09-22 | 2026-09-23 |
| Heck Fest | 2026-09-25 | 2026-09-24–2026-09-26 |
| Duluth Public Lands Day Celebration | 2026-09-26 | 2026-09-27 |
| Moorhead Community Gathering | 2026-10-07 | 2026-10-07–2026-10-08 |
| Saint Cloud Door Knock for BWCA Champs | 2026-10-12 | 2026-10-12–2026-10-13 |

The Duluth travel row still arrives September 27, after the master’s September 26 event. The master provides no replacement travel itinerary. The existing CSV also has no separate October 14 or October 28 row and describes the October 15 Mankato leg as “120 miles over 3 days” without an explicit start date. These gaps are preserved rather than filled with invented dates.

The headline distance remains the campaign’s 1,215 miles. The available GPX tracks total 1,146.3 miles and the listed travel rows total 1,103 miles; the event master does not reconcile these existing totals. The third GPX leg is named Moorhead to Saint Cloud, while its schedule group also includes the October 6 Detroit Lakes-to-Moorhead connection.

## Complete displayed schedule audit

All 32 rows below were checked in the browser against the generated data and local CSV. Event numbers refer to the master comparison above. Travel-only days have no counterpart in the event master.

| Leg | Ride dates | From → To | Listed miles | Master event stop |
| --- | --- | --- | --- | --- |
| Leg 1 | 2026-09-23 | Ely → Finland | 38 miles | 1 |
| Leg 1 | 2026-09-24–2026-09-26 | Finland → Two Harbors | 34 miles | 2 |
| Leg 1 | 2026-09-27 | Two Harbors → Duluth | 28 miles | 3 |
| Leg 1 | 2026-09-28 | Duluth → Cloquet | 20 miles | 4 |
| Leg 1 | 2026-09-29 | Cloquet → Floodwood | 35 miles | — |
| Leg 1 | 2026-09-30 | Floodwood → Grand Rapids | 50 miles | 5 |
| Leg 2 | 2026-10-01 | Grand Rapids → Deer River | 30 miles | — |
| Leg 2 | 2026-10-02 | Deer River → Cass Lake | 28 miles | — |
| Leg 2 | 2026-10-03 | Cass Lake → Bemidji | 15 miles | 6 |
| Leg 2 | 2026-10-04 | Bemidji → Park Rapids | 37 miles | — |
| Leg 2 | 2026-10-05 | Park Rapids → Detroit Lakes | 40 miles | — |
| Leg 3 | 2026-10-06 | Detroit Lakes → Moorhead | 50 miles | — |
| Leg 3 | 2026-10-07–2026-10-08 | Moorhead → Fergus Falls | 56 miles | 7 |
| Leg 3 | 2026-10-09 | Fergus Falls → Alexandria | 48 miles | — |
| Leg 3 | 2026-10-10–2026-10-11 | Alexandria → Saint Cloud | 70 miles | — |
| Leg 4 | 2026-10-12–2026-10-13 | Saint Cloud → Saint Cloud | Not listed | 8 |
| Leg 4 | 2026-10-15 | Saint Cloud → Mankato | 120 miles over 3 days | 9 |
| Leg 5 | 2026-10-16 | Mankato → Faribault | 40 miles | — |
| Leg 5 | 2026-10-17 | Faribault → Northfield | 15 miles | 10 |
| Leg 5 | 2026-10-18 | Northfield → Pine Island | 40 miles | — |
| Leg 5 | 2026-10-19 | Pine Island → Rochester | 20 miles | 11 |
| Leg 5 | 2026-10-20 | Rochester → Winona | 50 miles | 12 |
| Leg 6 | 2026-10-21 | Winona → Wabasha | 30 miles | — |
| Leg 6 | 2026-10-22 | Wabasha → Lake City | 32 miles | 13 |
| Leg 6 | 2026-10-23 | Lake City → Red Wing | 20 miles | — |
| Leg 6 | 2026-10-24 | Red Wing → Lakeville | 42 miles | 14 |
| Leg 6 | 2026-10-25 | Lakeville → Chaska | 30 miles | 15 |
| Leg 6 | 2026-10-26 | Chaska → Minnetonka / Excelsior | 15 miles | 16 |
| Leg 6 | 2026-10-27 | Minnetonka / Excelsior → Coon Rapids | 20 miles | 17 |
| Leg 6 | 2026-10-29 | Coon Rapids → Saint Paul | 25 miles | 18 |
| Leg 6 | 2026-10-30–2026-10-31 | Saint Paul → Saint Paul | 0 miles | — |
| Leg 6 | 2026-11-01 | Saint Paul → Stillwater | 25 miles | 19 |

## Verification

- `npm run check`: all 10 tests passed, including comparison of every event against the master fixture.
- `npm run build`: passed; Vite reports the existing large map bundle warning.
- Chrome browser audit passed at 1440×1000, 390×844, and 320×740. At every size, checked all 32 schedule cards, all 19 event details, their dates/day numbers/times/venues/addresses, all 18 RSVP button clicks, the additional Lakeville signup, all six route selectors and drawn paths, and all 19 map markers. No page errors or document overflow.
- All 18 master RSVP URLs plus the additional Lakeville signup returned HTTP 200 with event-specific page titles. Navigation was exercised without submitting forms.
- Additional browser checks passed at 1440, 390, and 320 pixels: no horizontal overflow inside any event card or detail panel; RSVP links are focusable and reachable, keyboard tab order reaches the schedule RSVP, and the Lakeville event hash restores the selected event and signup link (in Map view on mobile).
- Strengthened the master comparison to require exact venue names and compare both street addresses supplied by the master, allowing equivalent suite notation and existing ZIP codes. All 10 tests passed again.
- `git diff --check`: passed.

Changes are local and uncommitted. Preview: http://127.0.0.1:4173/.

## Subsequent RSVP presentation update

Per the final UI direction, RSVP controls now appear only in event popouts. The scrolling schedule has no RSVP buttons or pending-link messages. The popout uses “RSVP Here” with the existing warm gold accent, pine text, and an arrow. Earlier schedule-button checks above describe the initial implementation. The updated production build and desktop/mobile browser checks (1440 and 320 pixels) passed: all 19 event cards remain, the list has no RSVP controls, and selecting an event opens its popout with the gold RSVP link intact.

## Subsequent Ely update

Re-read the live Events List tab on September 9, 2026 and added its newly supplied Ely RSVP URL: https://secure.everyaction.com/bRWw6xnPwUWh8Xw4QzDwyA2. Ely’s date, time, venue, and description already matched the document. Updated the source data and master fixture; all 19 events now have RSVP links. Earlier verification counts of 18 links describe the initial audit.

## Subsequent Saint Cloud update — September 14, 2026

Per the campaign update, the Saint Cloud stop moved to Monday, October 12 (Day 21), at 5:00 PM CT, with the location displayed simply as “St. Cloud.” Updated the event source and maintained audit fixture, and moved event stop 8 to the October 12–13 Saint Cloud schedule row in Leg 4. The tables above reflect this change.

## Subsequent Ely time update — September 21, 2026

Per the campaign update, the Ely Launch Event on September 22 now starts at 5:00 PM instead of 4:00 PM and still ends at 7:00 PM. Updated the event source and maintained audit fixture; the table above reflects this change.
